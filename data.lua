...
function startCollectingTrial(participantID,referenceAngle)
	assert(not rows[#rows] or rows[#rows]["Millisecond Duration"],"Previous data collection not completed!")
	...
	return function ()
		rows[#rows]["Millisecond Duration"]=system.getTimer() - startMillis
...
function zipData(participantID,onComplete)
	local filename=("%s_%s_datacollector_data.zip"):format(tostring(participantID),os.date("%d-%m-%Y"))
	zip.compress({
		zipFile=filename,
